...
//		this.permalink = config.getPage("permalink");
...
//		PathUtils.checkDir(themeDir, PathUtils.Strategy.THROW_EXCEPTION_IF_NOT_EXISTS);
    ...
    void prepare() {
        ...
        if (cache) {
            ...
            pageCache = cacheManager.getCache("pages");

            if (pageCache == null) {
                throw new IllegalArgumentException("'pages' cache not defined");
            }
        }
    }
    ...
    void read() {
        ...
        final SourceDirectoryWalker sourceDirectoryWalker = factory.getSourceDirectoryWalker();
        ...
        final SourceVisitor sourceVisitor = new SourceVisitor() {
            ...
            public void visit(Origin origin) {
                readStaticFile(origin);
            }
        };

        final OriginVisitor staticFileVisitor = new OriginVisitor() {
            @Override
            public void visit(Origin origin) {
                readStaticFile(origin);
            }
        ...
        for (final File src : sources) {
            tasks.add(new Runnable() {
                ...
                public void run() {
                    ...
                    sourceDirectoryWalker.walk(src, fileFilter, sourceVisitor);
                }
            });
        }
        ...
        for (final File assetDir : assets) {
            tasks.add(new Runnable() {
                ...
                public void run() {
                    ...
                    sourceDirectoryWalker.walk(assetDir, fileFilter, staticFileVisitor);
                }
            });
        }
        ...
        //All other walkers
        List<SourceWalker> sourceWalkers = factory.getSourceWalkers();
        if (sourceWalkers != null && !sourceWalkers.isEmpty()) {
            for (SourceWalker walker : sourceWalkers) {
                log.debug("Walk source by: {}", walker);
                walker.walk(sourceVisitor);
            }
        }
        ...
        if (log.isTraceEnabled()) {
            for (Page page : allPages) {
                System.out.println("==> " + page.getSource().getOrigin());
            }
        }
    }

    private void readStaticFile(Origin origin) {
        log.debug("Reading static file {}", origin);
        staticFiles.add(new StaticFileImpl(SiteImpl.this, origin));
    }
...
//    }
    ...
    private void readSource(Source src) {
        Origin origin = src.getOrigin();
        log.debug("Reading source [{}] {}", origin.getPath(), origin.getName());
        ...
        if (!draft || (draft && showDrafts)) {
            ...
            if (origin instanceof FileOrigin) {
                try {
                    ...
                    String pathFromBasedirToFile = PathUtils.getRelativePath(basedir, ((FileOrigin) origin).getSourceDirectory())
                            + origin.getPath() + "/" + origin.getName();
                    page.set("pathFromBasedirToFile", pathFromBasedirToFile);
                }catch (Exception e){
                    if(log.isTraceEnabled()){
                        log.trace("Process 'pathFromBasedirToFile' failed.", e);
                    }else{
                        log.debug("Process 'pathFromBasedirToFile' failed.");
                    }
                }
            }
    ...
    void convert() {
        ...
        taskExecutor.run(allPages, new RunnableTask<Page>() {
            public void run(Page page) {
                ...
                Converter converter = getConverter(page.getSource());
    ...
    void render() {
        ...
        taskExecutor.run(allPages, new RunnableTask<Page>() {
            public void run(Page page) {
                ...
                page.render(renderer, factory.getHighlighter(), rootMap);
...
//			}
    ...
    void write() {
        ...
        log.info("Writing {} pages to {}", allPages.size(), dest);
        taskExecutor.run(allPages, new RunnableTask<Page>() {
            @Override
            public void run(Page input) {
                input.write(dest);
            }
        });

        if (!staticFiles.isEmpty()) {
            log.info("Writing {} static files to {}...", staticFiles.size(), dest);
            taskExecutor.run(staticFiles, new RunnableTask<StaticFile>() {
                @Override
                public void run(StaticFile input) {
                    input.write(dest);
                }
            });
        }
    ...
    public List<Page> getPages() {
        Collection collection = collections.get("page");
        if (collection != null) {
            List<Page> pages = collection.getPages();
    ...
    public List<Post> getPosts() {
        ...
        if (collection != null) {
            List<Page> pages = collection.getPages();
            return ImmutableList.copyOf(PageUtils.unwrap(pages, Post.class));
        }
        return Collections.emptyList();
    }
    ...
    @Override
    public synchronized Page addPage(Page page) {
    ...
    static class AllPages extends ArrayList<Page> implements List<Page> {
        public AllPages() {
            super(100);
        }

        AllPages addPage(Page page) {
            super.add(page);
            return this;
        }
        ...
        public boolean add(Page page) {
            throw new UnsupportedOperationException("Call Site#addPage(Page) method instead.");
        }

        @Override
        @Deprecated
        public boolean addAll(java.util.Collection<? extends Page> c) {
            throw new UnsupportedOperationException("Call Site#addPage(Page) method instead.");
        }

        @Override
        @Deprecated
        public boolean addAll(int index, java.util.Collection<? extends Page> c) {
            throw new UnsupportedOperationException("Call Site#addPage(Page) method instead.");
        }
